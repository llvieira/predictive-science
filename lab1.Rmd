...
despesasOrdenadasPorUso %>% ggplot(aes(x = reorder(tipoDespesa, vezesUsado), y = vezesUsado, fill = tipoDespesa)) + labs(x='Tipo Despesa', y='Quantidade Usada') + geom_col(width = 0.7) + coord_flip()  +guides(fill = guide_legend('Tipo Despesa'))

despesasOrdenadasPorGasto %>% ggplot(aes(x = reorder(tipoDespesa, totalGasto), y = totalGasto, fill = tipoDespesa)) + labs(x='Tipo Despesa', y='Gasto total') + geom_col(width = 0.7) +guides(fill = guide_legend('Tipo Despesa'))
